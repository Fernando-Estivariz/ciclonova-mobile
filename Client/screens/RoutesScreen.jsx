...
const RoutesScreen = ({ navigation }) => {
    ...
    // Iniciar ruta (función corregida)
    const startRoute = (route) => {
        console.log('Starting route:', route); // Debug log
        ...
        Alert.alert(
            'Iniciar Ruta',
            `¿Quieres comenzar la ruta "${route.name}"?`,
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    ...
                    onPress: () => {
                        try {
                            console.log('Navigating to RouteNavigation with data:', route); // Debug log
                            // Navegar a la pantalla de navegación de ruta
    ...
    // Iniciar navegación libre (sin ruta predefinida)
    const startFreeNavigation = () => {
        console.log('Starting free navigation'); // Debug log
        ...
        try {
            // Crear una ruta libre usando la ubicación actual como punto de inicio
            const freeRoute = {
                id: 'free-navigation',
                ...
                startLocation: currentLocation, // Agregar ubicación de inicio explícita
                createdAt: new Date().toISOString().split('T')[0],
            };

            console.log('Navigating to RouteNavigation with free route:', freeRoute); // Debug log
            navigation.navigate('RouteNavigationScreen', {
                routeData: freeRoute
            });
        } catch (error) {
    ...
    // Iniciar creación de ruta (función corregida)
    ...
    const finishRouteCreation = () => {
        if (routeCoordinates.length < 2) {
            Alert.alert(
                'Ruta Incompleta',
                'Necesitas al menos 2 puntos para crear una ruta. ¿Qué deseas hacer?',
                [
        ...
        Alert.prompt(
            'Nombre de la Ruta',
            'Ingresa un nombre para tu nueva ruta:',
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    ...
                    onPress: async (name) => {
                        if (name && name.trim()) {
                            ...
                            if (saved) {
                                ...
                                // Iniciar la ruta inmediatamente
                                setTimeout(() => {
                                    startRoute(newRoute);
                                }, 500);
                            } else {
                ...
                <View style={styles.header}>
                    <View style={styles.headerTop}>
                        <View>
                            ...
                            <Text style={styles.subtitle}>Descubre nuevas aventuras</Text>
                        </View>
